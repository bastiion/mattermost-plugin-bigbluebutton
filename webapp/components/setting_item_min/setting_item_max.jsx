...
export default class SettingItemMax extends React.PureComponent {
    ...
    onKeyDown = (e) => {
        if (this.props.shiftEnter && e.keyCode === 13 && e.shiftKey) {
            return;
        }
        if (this.props.disableEnterSubmit !== true && isKeyPressed(e, ['Enter', 13]) && this.props.submit && e.target.tagName !== 'SELECT' && e.target.parentElement && e.target.parentElement.className !== 'react-select__input' && !e.target.classList.contains('btn-cancel') && this.settingList.current && this.settingList.current.contains(e.target)) {
            this.handleSubmit(e);
        }
    }
